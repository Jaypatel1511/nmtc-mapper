...
| eligibility_status | str | verified-eligible / verified-ineligible / not-found / not-covered-territory / geocode-failed — exported as `nmtcmapper.ELIGIBILITY_STATUS_VALUES` |
...
|----------|------|-------------|
| distress_description | str | The string `summary()` prints as *Description* — selected from `eligibility_status` first (`not-covered-territory` reads the coverage wording, "Not covered — outside this table's universe, NOT a lookup miss"), otherwise the expansion of `distress_level` |
